...
|---------|---------|--------|----------|--------|
| [enhancement-005](enhancement-005-common-header-component.md) | 共通ヘッダーコンポーネントの導入 | Medium | 🟢 完了 | 2025-10-22 |

### UX（ユーザー体験）

| チケット | タイトル | 優先度 | ステータス | 作成日 |
|---------|---------|--------|----------|--------|
| [enhancement-006](enhancement-006-remove-preview-footer.md) | プレビュー画面のフッター削除 | Medium | 🟢 完了 | 2025-10-22 |
...
- 実績時間: 20分
- 改善効果: ConditionalFooterコンポーネントを作成し、/previewパスでのみ共通フッターを非表示に。プレビュー画面がシンプルになり、作成したUIに集中できる環境を実現。プレビュー情報フッター（Widget数等）は維持し、必要な情報は引き続き表示
...
- 実績時間: 30分
- 改善効果: ヘッダー実装箇所を4ファイルから1ファイルに削減（-75%）。variantプロップで2つのスタイルを持つ共通コンポーネントを作成し、フッターと同様のアーキテクチャパターンで統一。コード重複を完全に排除し、保守性が大幅に向上
